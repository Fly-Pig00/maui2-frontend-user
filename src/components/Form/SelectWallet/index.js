...
const FIAT = [
  ...
  {img: 'bg-common-transfer-from', symbol: 'BRL', fee: '2'},
];
...
function SelectWallet({isCrypto = false, id, className, label, selectedSymbol, onChange}) {
  ...
  const dropdown_crypto = _.map(CRYPTO, item=>{
    return (<div
      key={item.symbol}
      title={item.desc}
      className={`cursor-pointer p-[3px] pl-[10px] pr-[10px] bg-[#DDE5F5] dark:bg-[#282828] hover:bg-[#8D95A5] dark:hover:bg-black hover:text-[#00DDA2] border border-[#F6F8FA] text-center ${selected.index !== item.index?'text-black dark:text-white':'text-[#00DDA2]'}`}
      onClick={handleDropdownSelect.bind(this, item.index)}
    >
      <div className="flex items-center p-3 justify-between">
        <div className={`${item.img} bg-cover bg-center w-[30px] h-[30px]`} />
        <div className="ml-[10px] text-[#767070] dark:text-[#CCCDCD] text-left">{item.symbol}</div>
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        ...
        <div className="ml-[10px] flex flex-col">
          ...
          <div className={`text-[#6B8CFF] text-[14px]`}>+{selected.fee}% fee</div>
        </div>
        {isCrypto ? 
